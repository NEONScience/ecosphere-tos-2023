...
DT::datatable(
  ...
  caption = "CDW volume density estimates by plotID for selected sites and eventIDs."
)
...
#   Assign the same eventID to each record created within a 14 day window
mam_plotNight_nodups <- mam_plotNight_nodups %>% 
  dplyr::group_by(siteID) %>% 
  dplyr::mutate(
    date = lubridate::ymd(collectDate),
    group = cut(date, "14 days")
  ) %>%
  dplyr::ungroup() %>%
  dplyr::mutate(
    eventID = paste0(siteID, "_", group)
  )
...
#   Define core fields for analysis
...
``` {r mamDensityCalculation, results='asis', message=FALSE}
...
#--> Kate, what are units of output? # individuals per hectare? --------------------------------------------->####
...
### Use functions to calculate MNKA at plot scale and site scale for same years for which CDW volume data were calculated
#--> begin again here
