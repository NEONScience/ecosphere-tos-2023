...
#   Third, join tempCDW data with mamPlots and create a 'subtype' column to identify colocated sampling
tempCDW <- tempCDW %>%
...
#   over the 5-year CDW Tally measurement interval; 
...
#   Output: Two unresolveable duplicates flagged with duplicateRecordQF=2, which indicates records that are
#   identical based on primary keys but contain different values in other fields; see ?neonOS::removeDups
...
#   within the plot. To do this, temporarily remove records for transects with no logs, and remove
#   records with samplingImpractical != "OK".
...
#   Second, identify plots where all three transects do not have logs and bind to log count output from above
noLogs <- filterCDW %>%
