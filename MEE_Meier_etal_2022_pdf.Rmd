...
#   Define list of needed CRAN packages
cranPackages <- c(
  "tidyverse",
  "devtools", # Load packages from Github not available on CRAN
  "data.table",
  "knitr", # Table display tools for pdf output
...
#   Define list of needed Github packages
gitPackages <- c(
...
### Retrieve CDW Tally data for last 6 years from NEON Data Portal for all sites and
### dates
...
##  Save local copy of data for faster subsequent work
...
#   Summarise data to identify 5-6 sites with recent CDW Tally data from Distributed
#   plots that can be paired with MAM data; for demonstration purposes, goal is to 
#   choose a small number of forested sites that span a range of habitats.
summaryDist <- cdw %>%
...
#   Filter CDW data to selected sites, filter to most recent eventID, remove 
#   unneeded columns
tempCDW <- cdw %>%
  dplyr::filter(
...
#   'HARV_001.mammalGrid.mam'); the goal is to check whether each CDW namedLocation 
#   has a colocated MAM namedLocation. The structure of the namedLocation value for 
#   any NEON data product can be determined by downloading data and checking the 
#   namedLocation field.
...
#   function will return a value from the NEON API; if a CDW namedLocation is not also
#   a MAM namedLocation, the function does not return a value. Note: The 'data' argument
...
### Retrieve small mammal box trapping data for same sites and date range as retrieved
### for CDW
...
mam.req <- httr::GET(
  paste(
    "https://data.neonscience.org/api/v0/taxonomy/?taxonTypeCode=SMALL_MAMMAL",
    "verbose=true",
    "offset=0",
    "limit=1000",
    sep = "&"
...
### Check for individualID duplicates; duplicate checks are important because log 
### counts are used to estimate CDW volume
...
tempCDW <- tempCDW %>%
  dplyr::mutate(
    individualID = case_when(
      is.na(individualID) & targetTaxaPresent=="Y" ~ paste(
        "NEON.CDW", domainID, plotID, logID, sep = "."),
      TRUE ~ individualID
    ),
...
#   Distributed and Tower plots due to distant parcels of land used for each plot type
#   and different forest types. The RMNP values in this table are specific to 
#   Distributed plots since we wish to compare data with Small Mammal data.
cdwParam <- read.csv(file = "cdw_siteParamLookup.csv", header = TRUE)

#   Join look-up table values with CDW data and flag records with incorrect volume
#   factor (i.e., f-value)
tempCDW <- tempCDW %>%
  dplyr::left_join(
...
### Tally QC checks: Verify that logs in the dataset should be tallied based on 
### protocol criteria
...
#   roundDiameter = round diameter of the log (cm); i.e., the equivalentLogDiameter field
#   transectNum = the number of transects at a survey point (3 for NEON plots),
#   fValue = site-level F value, taken from the volumeFactor field in NEON data
tempCDW <- tempCDW %>%
  dplyr::mutate(
...
##  Filter out flagged records for logs that should not have been tallied
filterCDW <- tempCDW %>%
  dplyr::filter(
    limDistQF==0,
    logDistQF==0
...
#   Create custom nightuids for duplicates identified above that are missing primary
#   key value
...
# Two lines above mean that trapID and individualCode are the source of the NAs, which
# is expected based on the protocol.
...
# No records in the set, so these unresolved duplicates will not impact the intended
# analyses.
...
#   log was tallied within the plot, and remove samplingImpractical != "OK"
...
##  captures have the correct 'trapStatus' - i.e., check if tagIDs exist but trap status
...
##  Fourth, following the 'Minimum Number Known Alive' (MNKA) approach (Slade & Blair
##  2000), generate a column of all of the unique tagIDs included in the dataset
uTags <- captures %>%
  dplyr::select(tagID) %>%
...
##  To avoid time-consuming processing, save a copy of `for()` loop output
...
### Calculate MNKA per plot per bout for all data in processed capsNew dataset and
### select 3-year window of data appropriate for each site in order to match up with
### CDW volume data.
...
#   Create table of MAM data within 3-year windows by site based on year of CDW
#   volume data
...
#   Calculate plot-specific MNKA across bouts: Join plotBoutMNKA and mamSiteYears, filter
#   to records within 3-year window centered on CDW measurement year, and determine
#   maximum small mammal abundance for each plot, as well as other per plot metrics.
...
> Figure 1. Correlation between CDW Volume and Max Small Mammal Abundance at the plot
> scale across forested sites.
...
> Figure 2. Correlation between CDW Volume and Max Small Mammal Abundance by plot
> within site.
...
> Figure 3. Correlation between CDW Volume and average max Small Mammal Abundance
> across sites.
...
- Coarse Downed Wood volume (m^3^/ha) and Small Mammal abundance (count/ha) can each be calculated at the plot-level, and spatially colocated plots are identified by a shared `plotID`. 
    - It is possible to identify shared `plotID` values programmatically using the `geoNEON::getLocByName()` function (lines 165-170), or by downloading the CDW and Small Mammal datasets and joining via the `plotID` (lines 883-903).
